(function () {
    "use strict";
    var wqs_3_xola = angular.module("wqs_3_xola", ['ngAnimate','angular.filter',]);


// config
    wqs_3_xola.config(['$httpProvider', function( $httpProvider) {
...
// controler
    wqs_3_xola.controller('wqs_search_controller', function ($scope, $http, $q, dataService, dataServiceAjax, dataServiceAjaxmore, $timeout, $filter) {
...
//dataService starttime and endtime get URLparametr
    wqs_3_xola.factory('dataService', function($http, $q, $filter){
        return{
...
// dataServiceAjax for click (!not cleare)
    wqs_3_xola.factory('dataServiceAjax', function($http, $q, $filter){
        return{
            getData: function(productCode,startTime,endTime){
                var deferred = $q.defer();
...
// dataServiceAjaxmore for infinite
    wqs_3_xola.factory('dataServiceAjaxmore', function($http, $q, $filter){
        return{
            getData: function(productCode,startTime,endTime){
                var deferred = $q.defer();
...
/* filters */
	...
	wqs_3_xola.filter("asDate", function ($filter) {
	    return function (input) {
	        return $filter('date')(new Date(input), 'yyyy-MM-dd', 'UTC');
	    }
	});
 	wqs_3_xola.filter("asTime", function ($filter) {
	    return function (input) {
	        return $filter('date')(new Date(input), 'hh:mm a', 'UTC');
	    }
	});
	wqs_3_xola.filter("asDateTitle", function ($filter) {
	    return function (input) {
	        return $filter('date')(new Date(input), 'dd MMMM', 'UTC');
	    }
	});
	wqs_3_xola.filter("asDateTitleYears", function ($filter) {
	    return function (input) {
	        return $filter('date')(new Date(input), 'dd MMMM yyyy', 'UTC');
	    }
	});
	wqs_3_xola.filter("trust", ['$sce', function($sce) {
	...
	wqs_3_xola.directive("otcScripts", function() {
